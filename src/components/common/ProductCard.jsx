...
useEffect(() => {
  ...
  return (
    <div className="relative w-70 cursor-pointer" onClick={handleCardClick}>
      {!isLoading && (
        <button onClick={handleCartToggle} className={`absolute left-full top-6/7 transform -translate-y-1/2 ml-4 h-24 w-9 rounded text-xs tracking-tight flex items-center justify-center hover:bg-opacity-90 ${isInCart ? "bg-[#8f6865] text-white" : "bg-[#b398a5] text-white"}`} title={isInCart ? "Remove from Cart" : "Add to Cart"}>
          <span className="vertical-text"> {isInCart ? "Remove" : "Add to Cart"}</span>
        </button>
      )}
      ...
      <div className="flex flex-col rounded-xl bg-[#cfcaba] h-[400px] w-70 shadow-md">
        ...
        <div className="h-[300px]">
          <img src={image || "/Background/ExampleCard.jpg"} className="h-full w-full rounded-xl" alt={name}/>
        </div>
